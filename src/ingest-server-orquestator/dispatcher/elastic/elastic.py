...
OPEN_RAG_MAPPINGS = {
    ...
    "_meta": {
        "description": (
            ...
            "chunk text is stored once in content as semantic_text. Lexical BM25 "
            "fields are generated by the ingest pipeline and excluded from _source."
        ),
        ...
        },
        {
            "script": {
                ...
                "source": (
                    ...
                    "if (ctx.clean_title == null && ctx.title != null) { "
                    "ctx.clean_title = ctx.title; "
                    "} "
                    "if (ctx.content != null) { "
...
class ElasticsearchDispatch(AbstractDispatcher):
    ...
    def _pipeline_exists(self) -> bool:
        from elasticsearch import NotFoundError

        try:
            self._client.ingest.get_pipeline(id=self.pipeline_name)
