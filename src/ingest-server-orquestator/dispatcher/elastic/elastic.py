...
OPEN_RAG_MAPPINGS = {
    ...
    "_meta": {
        "description": (
            ...
            "chunk text is stored in content. Dense content_dense, sparse "
            "content_sparse, and lexical content_lex fields are generated by "
            "the ingest pipeline, searchable, and excluded from _source. "
            "Cleaned titles and Docling headings stay in _source and are "
            "indexed as sparse semantic_text."
        ),
        ...
        "lexical_routing": (
            ...
            "one searchable content_lex.<lang> field for language-specific BM25 "
            "analysis. Those large lexical copies are excluded from _source."
        ),
    ...
    "properties": {
        "content": CHUNK_CONTENT_FIELD,
        "content_dense": SEMANTIC_TEXT_FIELD,
        ...
        },
        {
            "script": {
                ...
                "source": (
                    ...
                    "} "
                    "if (ctx.content_dense != null) { "
                    "ctx.content_dense = escapeText(ctx.content_dense); "
                    "} "
        ...
        },
        {
            "script": {
                ...
                "source": (
                    ...
                    "if (ctx.content_dense == null) { ctx.content_dense = ctx.content; } "
...
def build_open_rag_mappings(inference_id: str) -> dict[str, Any]:
    ...
    mappings["properties"]["content"] = deepcopy(CHUNK_CONTENT_FIELD)
    mappings["properties"]["content_dense"] = deepcopy(SEMANTIC_TEXT_FIELD)
    ...
    def _ensure_index(self) -> None:
        if bool(self._client.indices.exists(index=self.index_name)):
            self._client.indices.put_mapping(
                ...
                properties=deepcopy(RECOVERABLE_DOCUMENT_FIELD_MAPPINGS),
            )
            return
